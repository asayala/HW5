...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        ...
        // Try to place the key-value pair, displacing as needed
        while (count < CAPACITY) {
            // If the position at pos1 is empty, insert here
            if (table[pos1] == null) {
            ...
            // Swap the current item with the one at pos1
            Bucket<K, V> temp = table[pos1];
            table[pos1] = new Bucket<>(displacedKey, displacedValue);
            displacedKey = temp.getBucKey();
            displacedValue = temp.getValue();

            // Alternate between positions based on hash1 and hash2
            pos1 = (pos1 == hash1(displacedKey)) ? hash2(displacedKey) : hash1(displacedKey);
            count++;
        }

        // If a cycle is detected, rehash and try to insert the original item again
        rehash();
